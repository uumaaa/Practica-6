...
def calculate_chain_code(image):
    ...
    for r in range(height):
        for c in range(width):
            if image[r][c] == 255:  # Assuming 0 represents the background, change to your background value if necessary
                start_y = r
                start_x = c
...
def dibujar_borde(image, chain_code):
    #Color imagen
...
def encontrar_menor_magnitud(chain_code):
    menor_magnitud = float('inf')  # Inicializa la menor magnitud como infinito para asegurarse de encontrar un valor menor.
    ...
    for i in range(len(chain_code)):
        ...
        subcadena_str = ''.join(map(str, subcadena))  # Convierte la subcadena en una cadena de dígitos
...
def imprimir_cadena(chain_code):
   #Se normalizan las direcciones, el 3 pasa a ser el 0, el 4 el 1, ...
    ...
    menor_magnitud = encontrar_menor_magnitud(chain_code)
    print("minimum magnitude:", menor_magnitud)

    return chain_code, first_difference, menor_magnitud
...
if __name__ == "__main__":
    ...
    chain1, first_difference1, menor_magnitud1 = imprimir_cadena(chain_code.copy())
    ...
    chain2, first_difference2, menor_magnitud2 = imprimir_cadena(chain_code.copy())
